...
class PurchaseController
{
	...
	public static function getTotalPurchaseClient($client_id = int)
	{
		try {
			return Purchase::join('client_site_user as csu', 'csu.id', '=', 'client_site_user_id')->join('client_site as cs', 'cs.id', '=', 'csu.client_site_id')->join('client as c', 'c.id', '=', 'cs.client_id')->selectRaw('SUM(total) as total')->where('c.id', $client_id)->where('status', 1)->first();
		} catch (Exception $ex) {
			$data = array(
				'msg' => $ex->getMessage(),
				'class' => 'error',
	...
	public static function chipPurchaseClient($client_id = int)
	{
		try {
			return Purchase::join('client_site_user as csu', 'csu.id', '=', 'client_site_user_id')->join('client_site as cs', 'cs.id', '=', 'csu.client_site_id')->join('client as c', 'c.id', '=', 'cs.client_id')->selectRaw('SUM(poker_chip) as total')->where('c.id', $client_id)->where('status', 1)->first();
		} catch (Exception $ex) {
			$data = array(
				'msg' => $ex->getMessage(),
				'class' => 'error',
	...
	public static function editPurchase($data = array())
	{
		try {
			...
			$PortionPurchase->portion = $data['total'];
			$PortionPurchase->bank_id = $data['bank_id'];
			$PortionPurchase->purchase_id = $data['id'];
			$PortionPurchase->save();
			$PortionPurchaseTotal = PortionPurchaseController::getTotalPortionPurchase($Purchase->id);
